...
const NotesPanel = () => {
  const { currentQuestionIndex, bookmarks, toggleBookmark, notes, addNote, getNote } = useExamStore();
    ...
    <div className="p-4 border-b border-gray-300 bg-gray-50">
      <div className="flex justify-between items-center mb-3">
        ...
        <button
          ...
          className={`p-2 rounded-lg ${isBookmarked ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-200 text-gray-700'} hover:opacity-90 transition-opacity`}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">
            Question {currentQuestionIndex + 1} {isBookmarked && '(Bookmarked)'}
          </span>
        ...
        {!isEditing ? (
          <div className="min-h-[100px] p-3 bg-white border border-gray-200 rounded-lg">
            {noteText ? (
              <p className="text-gray-700 whitespace-pre-line">{noteText}</p>
            ) : (
              <p className="text-gray-400 italic">No notes for this question yet. Click "Add Note" to start.</p>
            )}
          </div>
        ) : (
          <textarea
            ...
            className="w-full min-h-[100px] p-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
